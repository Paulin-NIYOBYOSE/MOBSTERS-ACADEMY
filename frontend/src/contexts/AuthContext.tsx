...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const startRefreshInterval = () => {
      ...
      if (isInSession) {
        console.log(
          "[AuthContext] Skipping auth refresh - user is in active session"
        );
        return;
      }
      ...
      refreshInterval = setInterval(async () => {
        ...
        if (token && !document.hidden) {
          // Only refresh when tab is visible and token exists
          try {
    ...
    visibilityChangeHandler = () => {
      ...
      if (document.hidden && isInSession) {
        ...
        console.log(
          "[AuthContext] Tab visible again during session - keeping auth refresh paused"
        );
      } else if (!document.hidden && !isInSession) {
